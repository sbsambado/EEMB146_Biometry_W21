---
title: "Lab5: Outliers, Transformations, Nonparametric Tests"
...
One of our critical assumptions for one-sample and two-sample t-tests are that
...
# your code
parasitoid  <- read_csv("parasitoids.csv")
...
str(parasitoid)
dim(parasitoid) # 84 observations/rows, 2 variables/columns
summary(parasitoid)
...
ggplot(parasitoid, aes(x = para_hosts)) +
  geom_histogram(fill = "brown1") + # change the color of histograms
  theme_bw() # change background of ggplot
...
ggplot(parasitoid, aes(sample = para_hosts)) +
  geom_qq() +
  theme_classic() # another background type for ggplot()
...
qqPlot(parasitoid$log_para_hosts, main = "Log number of parasitoid hosts")
...
#Re-assess normality of transformed data
qqPlot(parasitoid$sqrt_para_hosts, main = "Sqrt number of parasitoid hosts")
...
median(hearing$hearing[hearing$mammal == "tiger"], na.rm=T) # na.rm is removing NAs
median(hearing$hearing[hearing$mammal == "lion"], na.rm=T) # na.rm is removing NAs
```
...
# HINT: dist_from_edge_m should be a "factor", use ?factor() for more info
